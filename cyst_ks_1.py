...
Original file is located at
    https://colab.research.google.com/drive/16hc_B_iUIgvZHgQEGQWiUgCUyxMGg3GD
"""

# DataSet: CYST SOMENTE COM AS CLASSES K e S
...
def checkFileExistance(filePath):
    try:
        ...
        return False
if checkFileExistance('/content/gdrive/My Drive/BancoDados/CYST_KS_1_num.dat'):
    import sys
...
# CYST_KS_1 : lista que contará os dados para estatística dos classificadores
...
CYST_KS_1_num = np.zeros(20, dtype=np.float64)
CYST_KS_1_num[0] = best_acc
CYST_KS_1_num[1] = xensemb
xvencedor=[]
tg = 2
# a seguir salvaremos os melhores scores obtidos
for z in range(3):
  CYST_KS_1_num[tg] = xval[z]
  
  tg += 1
for z in range(3):
  CYST_KS_1_num[tg] = xscor1[z]
  
  tg += 1
for z in range(3):
  CYST_KS_1_num[tg] = et_scor[z]
  
  tg += 1
for z in range(3):
  CYST_KS_1_num[tg] = es_scor[z]
  
  tg += 1
for z in range(3):
  CYST_KS_1_num[tg] = mt_scor[z]
  
  tg += 1
for z in range(3):
  CYST_KS_1_num[tg] = ms_scor[z]
  tg += 1
...
bs_total = as_total - 2
CYST_KS_1_class = np.empty(bs_total, dtype=np.float64)
for i in range(bs_total):
    if xvencedor[i] == xnome_class[0]:
        CYST_KS_1_class[i] = float(0.0)
    if xvencedor[i] == xnome_class[1]:
        CYST_KS_1_class[i] = float(1.0)
    if xvencedor[i] == xnome_class[2]:
        CYST_KS_1_class[i] = float(2.0)
    if xvencedor[i] == xnome_class[3]:
        CYST_KS_1_class[i] = float(3.0)
    if xvencedor[i] == xnome_class[4]:
        CYST_KS_1_class[i] = float(4.0)
    if xvencedor[i] == xnome_class[5]:
        CYST_KS_1_class[i] = float(5.0)
    if xvencedor[i] == xnome_class[6]:
        CYST_KS_1_class[i] = float(6.0)
    if xvencedor[i] == xnome_class[7]:
        CYST_KS_1_class[i] = float(7.0)
    if xvencedor[i] == xnome_class[8]:
        CYST_KS_1_class[i] = float(8.0)
    if xvencedor[i] == xnome_class[9]:
        CYST_KS_1_class[i] = float(9.0)
cs_total = as_total - 2
CYST_KS_1_qtd = np.zeros(18, dtype=np.float64)
CYST_KS_1_qtd[0]=n_prim
CYST_KS_1_qtd[1]=n_seg
CYST_KS_1_qtd[2]=n_ter
CYST_KS_1_qtd[3]=xn_prim
CYST_KS_1_qtd[4]=xn_seg
CYST_KS_1_qtd[5]=xn_ter
CYST_KS_1_qtd[6]=et_n_prim
CYST_KS_1_qtd[7]=et_n_seg
CYST_KS_1_qtd[8]=et_n_ter
CYST_KS_1_qtd[9]=es_n_prim
CYST_KS_1_qtd[10]=es_n_seg
CYST_KS_1_qtd[11]=es_n_ter
CYST_KS_1_qtd[12]=mt_n_prim
CYST_KS_1_qtd[13]=mt_n_seg
CYST_KS_1_qtd[14]=mt_n_ter
CYST_KS_1_qtd[15]=ms_n_prim
CYST_KS_1_qtd[16]=ms_n_seg
CYST_KS_1_qtd[17]=ms_n_ter
pprint(CYST_KS_1_num)
pprint(CYST_KS_1_class)
pprint(CYST_KS_1_qtd)

arq_est_num = "/content/gdrive/My Drive/BancoDados/CYST_KS_1_num.dat"
arq_est_class = "/content/gdrive/My Drive/BancoDados/CYST_KS_1_class.dat"
arq_est_qtd = "/content/gdrive/My Drive/BancoDados/CYST_KS_1_qtd.dat"
CYST_KS_1_num.tofile(arq_est_num)
CYST_KS_1_class.tofile(arq_est_class)
CYST_KS_1_qtd.tofile(arq_est_qtd)

a_num = np.fromfile(arq_est_num)
b_class = np.fromfile(arq_est_class)
c_qtd = np.fromfile(arq_est_qtd)
pprint(a_num == CYST_KS_1_num)
pprint(b_class == CYST_KS_1_class)
pprint(c_qtd == CYST_KS_1_qtd)
